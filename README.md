# Biblioteca 
###### Simple personal app storing digital books (pdf, epub, djvu) filtering by authors and categories
###### Created with Django and Bootstrap
#### Add books:
###### The manage.py comand "add_books" stores all the books contained in the folder "/media/authors_folder/"
###### Within that folder there should be a folder for each author
###### Also each author should contain a "images/" folder to store book's first page images
###### To correctly store books, authors and categories the files should be stored with the following format:
###### authors_folder/{author1} - {author2}/{book_title} @{category1} @{category2}.{file_format}
###### Folder example:
![Screenshot](screenshots/folder.png)
##### Homepage:
![Screenshot](screenshots/homepage.png)
##### Categories:
![Screenshot](screenshots/categories.png)
##### Authors:
![Screenshot](screenshots/authors.png)
##### Books:
![Screenshot](screenshots/books.png)

